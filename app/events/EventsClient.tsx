      ...
      <section className="pt-20 pb-8 px-4 md:pt-28 md:pb-16 md:px-6" style={{ background: "linear-gradient(160deg, #F8F6F2 0%, #FAF6EC 100%)" }}>
        <div className="max-w-7xl mx-auto">
          ...
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="font-display text-2xl md:text-5xl lg:text-6xl font-light text-roope-primary mb-2 md:mb-4 max-w-xl">
            Glamour for every
            <span className="italic text-gradient-gold"> occasion.</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            className="text-xs md:text-sm text-stone-warm max-w-lg font-light leading-relaxed">
            Party looks, group styling, fashion events, corporate shoots — Roopé delivers premium makeup for every kind of event.
          </motion.p>
        </div>
      </section>

      <section className="py-6 px-4 md:py-16 md:px-6">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-4 md:gap-10">
            ...
            <div className="card-luxury p-3.5 md:p-6 mb-4 md:mb-8">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                <button onClick={() => setGroupSize(Math.max(1, groupSize - 1))}
                  className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-pearl-300 flex items-center justify-center hover:border-champagne-DEFAULT transition-colors">
                  <Minus className="w-3.5 h-3.5 text-stone-warm" />
                </button>
                <span className="font-display text-lg font-semibold md:text-3xl md:font-light text-roope-primary w-10 md:w-12 text-center">{groupSize}</span>
                <button onClick={() => setGroupSize(groupSize + 1)}
                  className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-pearl-300 flex items-center justify-center hover:border-champagne-DEFAULT transition-colors">
                  <Plus className="w-3.5 h-3.5 text-stone-warm" />
                </button>
                ...
                {addons.map((addon) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-2.5 md:p-4 rounded-xl md:rounded-2xl text-left transition-all duration-200 ${selected ? "shadow-gold" : "border border-pearl-300"}`}
                      style={{
                    ...
                    >
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className={`w-4 h-4 md:w-5 md:h-5 rounded-full border-2 flex items-center justify-center ${selected ? "border-champagne-DEFAULT" : "border-stone-warm/30"}`}
                          style={{ background: selected ? "#C9A84C" : "transparent" }}>
                          {selected && <Check className="w-2.5 h-2.5 text-white" />}
                        </div>
                        <span className="text-xs md:text-sm font-medium text-roope-primary">{addon.label}</span>
                      </div>
                      <span className="text-xs md:text-sm text-stone-warm">+{formatPrice(addon.price)}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {/* Price summary */}
          <div>
            <div className="card-luxury p-4 md:p-7 sticky top-24">
              <p className="section-label mb-3.5 md:mb-5">Live Pricing</p>
              <div className="space-y-2 md:space-y-3 text-xs md:text-sm mb-4 md:mb-5">
                {base && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-roope-primary">{formatPrice(base.price * groupSize)}</span>
                  </div>
                )}
                {selectedAddons.map((id) => {
                  ...
                  return a ? (
                    <div key={id} className="flex justify-between">
                      ...
                      <span className="font-semibold text-roope-primary">+{formatPrice(a.price)}</span>
                    </div>
                  ) : null;
                })}
                {discount > 0 && (
              ...
              <button
                ...
                className="btn-primary w-full justify-center gap-1.5 py-2.5 text-xs md:py-4 md:text-sm"
              >
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 md:gap-4">
                    <div>
                      <label className="block text-xs md:text-sm font-medium text-stone-warm mb-1 md:mb-2">Event Date</label>
                  ...
                  <div>
                    <label className="block text-xs md:text-sm font-medium text-stone-warm mb-1 md:mb-2">Message (optional)</label>
                  ...
                  <button type="submit" disabled={submitting} className="btn-primary w-full justify-center py-2.5 md:py-4 gap-1.5 md:gap-2 text-xs md:text-sm disabled:opacity-60">
                    {submitting ? "Submitting..." : "Confirm Inquiry"} <Send className="w-3.5 h-3.5" />
                  </button>
                </form>
              )}
            </motion.div>
