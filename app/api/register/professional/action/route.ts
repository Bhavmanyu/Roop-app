...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (fetchError || !app) {
      ...
      return new NextResponse(renderHtmlError("Application Not Found", "The professional application could not be located in our partner database."), {
        headers: { "Content-Type": "text/html" },
        status: 404
      });
    }
    ...
    // Prevent re-processing if already resolved to save transactional email limits
    if (app.status !== "pending") {
      return new NextResponse(renderHtmlSuccess(
        app.name, 
        app.status.toUpperCase(), 
        `This application has already been processed as **${app.status.toUpperCase()}**. No further actions were taken.`,
        app.status === "accepted"
      ), {
        headers: { "Content-Type": "text/html" }
      });
    ...
    if (updateError) {
      ...
      return new NextResponse(renderHtmlError("Database Error", "Failed to update the application status in the database. Please try again."), {
        headers: { "Content-Type": "text/html" },
        status: 500
      });
    }

    // 3. Dispatch automated follow-up email to applicant
    try {
    ...
    return new NextResponse(renderHtmlSuccess(
      ...
      isAccepted 
        ? "We logged the approval in the database and sent a congratulations onboarding email to the artist. They will expect partner calls shortly."
        : "The application has been declined. A polite status update email has been sent to the candidate.",
      isAccepted
    ), {
      headers: { "Content-Type": "text/html" }
    });
